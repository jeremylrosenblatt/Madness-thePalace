...
const REGION_TABS = ['West','East','South','Midwest','FinalFour'] as const
type Tab = typeof REGION_TABS[number]
...
function MatchCard(props: {
  ...
  const pickableA = !!a && !!b
  const pickableB = pickableA

  return (
    <div className="match" style={{opacity: a && b ? 1 : .6}}>
      <div className="team">
        <span className="seed">{a ? a.seed : ''}</span>
        <button
          disabled={locked || !pickableA}
...
export default function MyBracket(props: {
  ...
  const pick = (matchId: string, winnerTeamId: string) => {
        ...
        <div className="card">
          ...
          <input
            ...
            onChange={(e)=>{
              ...
              props.onUpdateEntry({ ...props.entry, tiebreaker: v ? Number(v) : undefined })
            }}
            placeholder="e.g., 142"
            inputMode="numeric"
            disabled={locked}
  ...
  return (
    <div className="grid" style={{gap:12}}>
      ...
      <small className="muted">Dummy bracket structure is now full-round with automatic advancement. Next: PDF-like single-screen bracket on desktop.</small>
    </div>
  )
}
